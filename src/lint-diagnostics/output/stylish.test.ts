...
void test("formatStylishEntry: emits the head line and indented slice line", () => {
  const result = formatStylishEntry(makeFileFinding({ message: "say something" }));
...
void test("formatStylishEntry: collapses newlines in the message to single spaces", () => {
  const result = formatStylishEntry(
    makeFileFinding({ message: "first line\nsecond line\r\nthird line" }),
  );
  assert.equal(
    result,
...
void test("formatStylishEntry: null code renders as the parse-error placeholder", () => {
  const result = formatStylishEntry(
    makeFileFinding({
  ...
  assert.equal(result, "  1:11 Unexpected token. [parse-error]\n    ;");
});

void test("formatProjectStylishEntry: indented one-line shape with no L:C and no slice", () => {
  const result = formatProjectStylishEntry(makeProjectFinding({ message: "tsconfig msg" }));
...
void test("formatProjectStylishEntry: collapses newlines in the message to single spaces", () => {
  const result = formatProjectStylishEntry(
    makeProjectFinding({ message: "first\nsecond\r\nthird" }),
  );
  assert.equal(result, "  first second third [typescript(tsconfig-error)]");
});

void test("formatProjectStylishEntry: null code renders as the parse-error placeholder", () => {
  const result = formatProjectStylishEntry(makeProjectFinding({ code: null, message: "msg" }));
  assert.equal(result, "  msg [parse-error]");
});
...
void test("renderStylish: empty filename surfaces under the (project) placeholder heading", () => {
  const result = renderStylish([], [makeProjectFinding({ filename: "", message: "no path" })]);
...
void test("renderStylish: project entries with the same heading cluster under one section", () => {
  const a = makeProjectFinding({ filename: "tsconfig.json", message: "a" });
  const b = makeProjectFinding({ filename: "tsconfig.json", message: "b" });
