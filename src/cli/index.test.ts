...
void test("--help / -h: writes usage to stdout and returns 0", () => {
  for (const flag of ["--help", "-h"]) {
    const cap = createCapturingLogger();
    const code = runCli([flag], process.cwd(), cap.logger);
...
void test("--version / -v: writes 'lint-js <semver>' to stdout and returns 0", () => {
...
void test("argv parse error routes through the boundary as exit 2 + tagged stderr", () => {
  ...
  const cap = createCapturingLogger();
  const code = runCli(["--no-such-flag"], process.cwd(), cap.logger);
